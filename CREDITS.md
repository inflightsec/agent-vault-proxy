...
`agent-vault-proxy` is original work licensed under Apache-2.0 (see [`LICENSE`](./LICENSE)). A few projects influenced the design:
...
- **Bitwarden Secrets Manager** — the reference vault backend AVP ships against. Its client, [`bitwarden-sdk`](https://pypi.org/project/bitwarden-sdk/), is under Bitwarden's proprietary SDK license (not Apache-2.0); AVP keeps it an opt-in extra (`agent-vault-proxy[bitwarden]`) so the default install stays fully open source.
