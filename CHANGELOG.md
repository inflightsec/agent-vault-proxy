...
- `avp setup` cross-platform installer (Linux systemd / macOS launchd, isolated service user, append-only audit log); `--no-service` provisions without activating — the path for Ansible/managed supervision. `tests/setup-e2e/` bats suite + `e2e-setup` CI job assert the provisioned end state in a disposable container.
