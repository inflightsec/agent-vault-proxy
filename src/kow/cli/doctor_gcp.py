"""``kow doctor --probe-gcp`` — read-only Google Secret Manager scope report.
...
def run_gcp_probe(*, config_path: str | None) -> bool:
    ...
    if config_path is None:
        print("kow doctor --probe-gcp: --config <path> is required.", file=sys.stderr)
        return True
    try:
        config = load_config(config_path)
    except Exception as e:  # noqa: BLE001 — operator-facing CLI surface
        print(
            f"kow doctor --probe-gcp: cannot load config {config_path}: {type(e).__name__}: {e}",
            file=sys.stderr,
        )
        return True
    ...
    if backend_type != "gsm":
        print(
            f"kow doctor --probe-gcp: backend is {backend_type!r}, not 'gsm' — nothing to probe.",
            file=sys.stderr,
        )
        return True
    ...
    try:
        ...
        print(
            f"kow doctor --probe-gcp: cannot build gsm backend: {type(e).__name__}: {e}",
            file=sys.stderr,
        )
        return True
    ...
    # each secret is sent — even without `versions.access`. kow reads with its
    ...
    print()
    print("kow doctor --probe-gcp: GSM identity scope")
    for status, check, msg in rows:
        print(f"    {status:5s} {check:12s} {msg}")
    print()
    any_fail = any(status == "FAIL" for status, _, _ in rows)
    if any_fail:
        print("kow doctor --probe-gcp: one or more FAIL", file=sys.stderr)
    else:
        print("kow doctor --probe-gcp: report complete (WARN rows are advisory)")
    return any_fail
