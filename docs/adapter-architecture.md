...
**What ships today:** five backends. `bws` (Bitwarden Secrets Manager), `gsm` (Google Secret Manager), and `aws-secrets-manager` (AWS Secrets Manager — ADR-0038) are the networked production backends; `keychain` (macOS Keychain via `/usr/bin/security` — ADR-0046) is the local one, for a single Mac with no vault account — it stores nothing in plaintext, but its access boundary is the **user account**, not kow, which ADR-0046 explains in full; `static` reads `{name: value}` pairs from a plaintext YAML file and exists only for development, testing, and the docker-e2e harness (it refuses world-readable files and warns loudly when selected, never use it in production). The `SecretsBackend` protocol, the registry, and the discriminated `backend: {type, config}` form in `bindings.yaml` are all live as of v0.4.0. The remaining vault backends in the coverage matrix below are the design target, not yet shipped: the matrix exists to prove the protocol holds across substantially different vaults, and to give a contributor the map for adding one.
...
Of the vaults, `bws`, `gsm`, `aws-secrets-manager`, and the local `keychain` ship today (the `static` test backend aside). The rest are the design target: the matrix is the proof that one protocol plus a per-backend config block spans substantially different auth models (machine-account tokens, AppRole, IAM role chains, AAD credential chain, ADC).
...
| `keychain` **(shipped)** | `service`, `keychain?`, `secret_prefix?`, `self_check`, `timeout_seconds` | **macOS Keychain**, the only *local* production backend. Shells out to `/usr/bin/security` (absolute path, never `PATH`) against the file-based login keychain — zero third-party dependencies, nothing in plaintext on disk, viewable in Keychain Access. Writes go through `security -i` on **stdin**, never `argv`, and are read back and compared before the write is reported as successful. **Access boundary is the user account, not kow** — kow is not Developer-ID-signed under PyPI/Homebrew, so the ACL identity is `security` itself. Single-Mac only: the login keychain never syncs. Multi-line values refused. See [ADR-0046](adrs/ADR-0046-macos-keychain-backend.md). |
